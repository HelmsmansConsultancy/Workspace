...
def current_open_positions():
    ...
    tradeOrders: list[TradeOrder] = metatrader5Service.getOpenPositions(connected_account.id)
    click.echo(f"Gotten {len(tradeOrders)} trades")
    openPositions: list[OpenPosition] = databaseService.getOpenPositions(connected_account.id)
...
def open_positions(ctx: click.Context):
    ...
    while True:
        ...
        if bool(connected_account):
            openPositions = databaseService.getOpenPositions(connected_account.id);
